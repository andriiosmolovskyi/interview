...
class OneFrameScheduledCacheDecorator[F[_]: Concurrent](underlying: Algebra[F],
  ...
  // So by using scheduler with interval 2 minutes token will be used 720 times
  // In case when external service down we will retry each request 5 times
  // If the external service will up and running there will be threshold of max 5 seconds between
  // Job execution and service become available, in this case all new requests will be handled with getFromUnderlying method
  schedulerAdaptor.scheduleTask(() => Await.result(mapperF(cacheUpdateScheduleJob()), 2.minutes), 2.minutes)
  ...
  override def get(request: List[Pair]): F[Either[errors.Error, Map[Pair, Rate]]] =
    mapper(ratesCache.getAllFuture(request, it => mapperF(getFromUnderlying(it)))).attempt.map {
      case Left(error: errors.Error) => error.asLeft
      case Left(e) =>
        val error = cacheLookupError(request, e.getMessage)
        logger.error(error.getMessage)
  ...
  // Attempt limit is 24 because new scheduled task will be executed each 2 minutes and timeout on client is 5 seconds
  private def cacheUpdateScheduleJob(attempt: Int = 1): F[Unit] =
    getFromUnderlying(AllPossiblePairs)
      ...
      .recoverWith {
        ...
        case e if attempt <= 24 =>
          logger.error(e)(s"Cannot execute update cache scheduled job, num of attempts = $attempt")
          cacheUpdateScheduleJob(attempt + 1).void
        case e =>
          logger.error(e)(s"Cannot execute update cache scheduled job, limit of attempts exceeded")
          implicitly[Concurrent[F]].unit
      }
